...
export default function OEMCrossReferenceClient() {
  return (
    <div className="bg-slate-50 min-h-screen font-sans overflow-x-clip">
      ...
      <section className="pt-8 pb-24 relative overflow-hidden bg-blue-50/50">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-[1920px] relative z-10">
          ...
          >
            {[
              { name: 'Bendix', path: 'bendix', color: 'amber', region: 'North American' },
              { name: 'Meritor', path: 'meritor', color: 'amber', region: 'North American' },
              { name: 'Haldex', path: 'haldex', color: 'amber', region: 'North American' },
              { name: 'MGM', path: 'mgm', color: 'amber', region: 'North American' },
              { name: 'TSE', path: 'tse', color: 'amber', region: 'North American' },
              { name: 'ZF / WABCO', path: 'zf-wabco', color: 'navy', region: 'European / Global' },
              { name: 'Knorr-Bremse', path: 'knorr-bremse', color: 'navy', region: 'European / Global' },
              { name: 'SORL', path: 'sorl', color: 'navy', region: 'European / Global' }
            ].map((brand) => (
              <motion.div key={brand.name} variants={fadeInUp}>
                ...
                >
                  ...
                  <p className="relative z-10 text-slate-500 text-[10px] sm:text-xs lg:text-sm font-medium mb-4 sm:mb-6 lg:mb-8 flex-grow leading-tight">{brand.region}</p>
                  ...
                  <span className={`relative z-10 text-${brand.color}-600 font-extrabold text-[9px] sm:text-[11px] lg:text-[13px] uppercase tracking-widest flex items-center mt-auto`}>
                    View Parts <ArrowRight className="w-3 h-3 sm:w-3.5 sm:h-3.5 lg:w-4 lg:h-4 ml-1 sm:ml-1.5 lg:ml-2 transform group-hover:translate-x-1 transition-transform" />
                  </span>
                </Link>
              </motion.div>
            ))}
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-[1920px] relative z-10">
          ...
          <div className="mt-16 lg:mt-24 relative z-20">
            <div className="grid grid-cols-1 lg:grid-cols-3 2xl:grid-cols-4 gap-6 lg:gap-8">
              ...
              <Link href="/oem-cross-reference/database" className="bg-gradient-to-b from-navy-800 to-navy-900 border border-navy-700 p-6 lg:p-10 rounded-[2rem] hover:border-emerald-500/50 transition-all duration-500 shadow-xl shadow-navy-900/10 group flex flex-col relative overflow-hidden transform hover:-translate-y-2">
                ...
                <h3 className="relative z-10 text-2xl font-extrabold text-white mb-4 group-hover:text-emerald-400 transition-colors">Download Database</h3>
                <p className="relative z-10 text-navy-300 text-base leading-relaxed mb-8 flex-grow font-light">Full cross-reference database in PDF and CSV formats.</p>
