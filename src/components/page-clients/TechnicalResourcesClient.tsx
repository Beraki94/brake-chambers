...
export default function TechnicalResourcesClient() {
  ...
  const categories = useMemo(() => [
    {
      ...
      accent: 'amber',
      desc: 'Step-by-step OEM installation procedures, maintenance schedules, and safe caging procedures. Ensure your BRC chambers are installed correctly and maintained for maximum lifespan.',
      items: [
      ...
      cta: 'Browse Installation Guides',
      ctaLink: '/technical-resources/installation-maintenance'
    },
    {
      id: 'specifications',
      ...
      cta: 'Browse Specs & Sizing',
      ctaLink: '/technical-resources/specifications-sizing'
    },
    {
      id: 'safety',
      ...
      accent: 'emerald',
      desc: 'Essential compliance documents including CVSA criteria, FMVSS 121 standards, and safety warnings. Stay compliant with DOT and international safety regulations.',
      items: [
      ...
      cta: 'Browse Safety Docs',
      ctaLink: '/technical-resources/safety-compliance'
    },
    {
      id: 'engineering',
      ...
      accent: 'slate',
      desc: 'Advanced technical resources including CAD models, material specifications, and engineering whitepapers. For engineers and technical buyers who need in-depth product data.',
      items: [
      ...
      cta: 'Browse Engineering Resources',
      ctaLink: '/technical-resources/engineering-training'
    }
  ], []);

  // Sync scroll position with active category on the left
  useEffect(() => {
  ...
  // Smooth scroll handler that respects CSS scroll-margin-top
  const handleScrollToCategory = (e: React.MouseEvent<HTMLAnchorElement>, id: string) => {
    e.preventDefault();
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans overflow-x-clip">
      <PageHeader
        ...
        description="Access our comprehensive technical library. Download engineering specifications, installation procedures, safety compliance manuals, and training videos — all designed to help you install, maintain, and troubleshoot BRC brake chambers with confidence."
      ...
      >
        <Link href="/contact" className="inline-flex items-center text-amber-500 font-bold hover:text-amber-400 transition-colors uppercase tracking-widest text-sm mt-4">
          Contact BRC Engineering <ArrowRight className="w-5 h-5 ml-2" />
        </Link>
      </PageHeader>
      ...
      <div className="relative z-20 -mt-16 md:-mt-24">
        ...
        <section className="py-16 md:py-24 relative">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
            <div id="video-training" className="scroll-mt-32 lg:scroll-mt-40 bg-navy-950 rounded-2xl shadow-2xl overflow-hidden relative">
        ...
        <section className="py-16 md:py-24 relative bg-slate-50 border-t border-slate-200">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
            <div className="mb-10 lg:mb-16">
              <SectionHeader
                badge="Library Categories"
                title="Browse Technical Library by Topic"
                description="Find the technical resources you need. Each category contains PDFs, guides, and training materials for technicians, engineers, and fleet managers."
                align="left"
              />
            </div>
            ...
            <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 items-start relative">
              ...
              <div className="w-full lg:w-1/3 lg:sticky lg:top-32 self-start z-10">
                <motion.div
                  initial="hidden"
                  animate="visible"
                  variants={fadeInUp}
                    ...
                    <ul className="space-y-2 md:space-y-3">
                      {categories.map((category) => (
                        <li key={category.id}>
                          <a
                            href={`#${category.id}`}
                            onClick={(e) => handleScrollToCategory(e, category.id)}
                      ...
                      <div className="relative z-10">
                        ...
                        {category.cta && (
                          <div className="mt-8 pt-6 border-t border-slate-100">
                            <Link href={category.ctaLink} className={`inline-flex items-center gap-2 font-black transition-colors text-[13px] uppercase tracking-widest group
                              ${category.accent === 'amber' ? 'text-amber-600 hover:text-amber-500' : ''}
                              ${category.accent === 'navy' ? 'text-navy-600 hover:text-navy-500' : ''}
                              ${category.accent === 'red' ? 'text-red-600 hover:text-red-500' : ''}
                              ${category.accent === 'emerald' ? 'text-emerald-600 hover:text-emerald-500' : ''}
                              ${category.accent === 'slate' ? 'text-slate-600 hover:text-slate-500' : ''}
                            `}>
                              {category.cta} <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                            </Link>
                          </div>
                        )}
                      </div>
