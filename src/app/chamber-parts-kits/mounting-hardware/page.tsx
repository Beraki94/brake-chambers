...
export const metadata: Metadata = {
  title: 'Mounting Hardware & Brackets | BRC Manufacturer',
  description: 'Caging bolts, clamp bands, straight and threaded clevises for secure installation.',
};
...
export default async function SubCategoryPage(props: { searchParams?: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  let products = BRAKE_ACCESSORIES.filter(a => ['Caging Bolt', 'Clamp Band', 'Slack Adjuster'].includes(a.category));
    ...
    <div className="container mx-auto px-4 lg:px-8 py-8 max-w-7xl">
      <nav aria-label="Breadcrumb" className="mb-6">
        <ol className="flex items-center space-x-2 text-sm text-slate-500">
          ...
          <li><span className="text-slate-300">/</span></li>
          <li className="text-navy-900 font-semibold" aria-current="page">Mounting Hardware & Brackets</li>
        </ol>
      </nav>
      ...
      <div className="mb-8 rounded-3xl bg-navy-900 relative overflow-hidden shadow-2xl p-8 sm:p-12 border border-navy-800">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row items-start justify-between gap-8">
          <div className="max-w-3xl">
            <h1 className="text-3xl sm:text-5xl font-extrabold text-white mb-6 leading-tight">Mounting Hardware & Brackets</h1>
            <div className="text-navy-100 text-base sm:text-lg leading-relaxed space-y-4">
              <p>Don't compromise a new brake chamber installation by reusing fatigued hardware. We manufacture a full line of heavy-duty caging bolts, heavy-gauge steel clamp bands, and forged clevis assemblies to ensure a secure, safe installation.</p>
            </div>
          </div>
