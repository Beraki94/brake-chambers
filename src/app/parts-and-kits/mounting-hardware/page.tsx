  ...
  return (
    <div className="bg-slate-50 min-h-screen font-sans overflow-x-clip">
      <PageHeader 
        ...
        description="Don't compromise a new brake chamber installation by reusing fatigued hardware. We manufacture a full line of heavy-duty caging bolts, heavy-gauge steel clamp bands, and forged clevis assemblies to ensure a secure, safe installation."
        breadcrumbs={[
          { label: 'Home', href: '/' },
          { label: 'Chamber Parts Kits', href: '/parts-and-kits' },
          { label: 'Mounting Hardware & Brackets' }
        ]}
      />
          ...
          {products.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-4">
              {products.map(product => (
                <AccessoryListCard key={product.slug} product={product} categoryPath="parts-and-kits" />
              ))}
            </div>
          ) : (
