...
export const metadata: Metadata = {
  title: 'Drive Axle Service Chambers | BRC Manufacturer',
  description: 'Maximum stopping power service chambers designed for drive axles (Typically Type 30).',
};
...
export default async function SubCategoryPage(props: { searchParams?: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  let products = BRAKE_CHAMBERS.filter(c => c.category === 'Service Brake' && c.type.includes('30'));
  ...
  return (
    <div className="bg-slate-50 min-h-screen font-sans overflow-x-clip">
      <PageHeader 
        badge="Drive Axle Service"
        title="Drive Axle Service Chambers"
        description="The drive axles handle the majority of a commercial vehicle's stopping requirements. Our Drive Axle Service Chambers (typically Type 30) are built to deliver massive, consistent braking force."
        breadcrumbs={[
          { label: 'Home', href: '/' },
          { label: 'Service Chambers', href: '/service-chambers' },
          { label: 'Drive Axle Service Chambers' }
        ]}
      />
