...
const categoryMeta = `const CATEGORY_META: Record<string, { title: string, desc: string, name: string }> = {
  'type-30': { title: 'Type 30 Service Brake Chambers | Wholesale | BRC', desc: 'Factory-direct Type 30 service brake chambers. OEM replacements for heavy-duty commercial trucks.', name: 'Type 30 Service Brakes' },
  'type-24': { title: 'Type 24 Service Brake Chambers | Wholesale | BRC', desc: 'Factory-direct Type 24 service brake chambers. OEM replacements for heavy-duty commercial trucks.', name: 'Type 24 Service Brakes' },
  'type-20': { title: 'Type 20 Service Brake Chambers | Wholesale | BRC', desc: 'Type 20 service brake chambers. Factory direct wholesale pricing.', name: 'Type 20 Service Brakes' },
  'type-16': { title: 'Type 16 Service Brake Chambers | Wholesale | BRC', desc: 'Type 16 service brake chambers. Factory direct wholesale pricing.', name: 'Type 16 Service Brakes' },
  'long-stroke': { title: 'Long Stroke Service Brake Chambers | Wholesale | BRC', desc: 'Long stroke service brake chambers for extended safety margin. Direct replacements for Bendix and Meritor.', name: 'Long Stroke Service Brakes' }
};`;
...
const filterLogic = `
  ...
  if (slug === 'type-30') products = products.filter(c => c.type.includes('30'));
  else if (slug === 'type-24') products = products.filter(c => c.type.includes('24'));
  else if (slug === 'type-20') products = products.filter(c => c.type.includes('20'));
  else if (slug === 'type-16') products = products.filter(c => c.type.includes('16'));
  else if (slug === 'long-stroke') products = products.filter(c => c.strokeSize === 'Long Stroke');
`;
...
export default async function UnifiedServiceBrakesPage(
  props: { 
  ...
  if (meta) {
    ...
    return (
      <div className="container mx-auto px-4 lg:px-8 py-8 max-w-7xl">
        <Breadcrumb items={[{ label: 'Service Chambers', href: '/service-chambers' }, { label: meta.name }]} />
          ...
          <div className="flex-grow">
            ...
            {products.length > 0 ? (
              ...
              <div className="bg-white p-8 rounded-xl border border-slate-200 text-center">
                ...
                <Link href="/service-chambers" className="text-amber-600 font-bold hover:underline">View All Service Chambers</Link>
              </div>
            )}
        ...
        <Script id={\`service-chambers-\${slug}-schema\`} type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify([
          {
            "@context": "https://schema.org",
            "@type": "CollectionPage",
            "name": meta.title,
            "description": meta.desc,
            "url": \`https://www.brcbrakechambers.com/service-chambers/\${slug}\`,
          ...
          },
          {
            ...
            "itemListElement": [
              ...
              { "@type": "ListItem", "position": 2, "name": "Service Chambers", "item": "https://www.brcbrakechambers.com/service-chambers" },
              { "@type": "ListItem", "position": 3, "name": meta.name, "item": \`https://www.brcbrakechambers.com/service-chambers/\${slug}\` }
            ]
          }
        ]) }} />
      </div>
  ...
  if (product) {
    return (
      <div className="container mx-auto px-4 lg:px-8 py-8 max-w-6xl">
        <Breadcrumb items={[{ label: 'Service Chambers', href: '/service-chambers' }, { label: product.name }]} />
...
fs.mkdirSync('src/app/service-chambers/[slug]', { recursive: true });
fs.writeFileSync('src/app/service-chambers/[slug]/page.tsx', [
  imports,
  categoryMeta,
  metadataBlock,
  componentStart,
