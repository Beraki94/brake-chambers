...
with open('src/features/technical-resources/components/TechnicalResourcesClient.tsx', 'r', encoding='utf-8') as f:
    content = f.read()
...
content = re.sub(r'  const categories = \[\s*\{.*?\];\s*return', categories_str + '\n\n  return', content, flags=re.DOTALL)
...
content = content.replace("description=\"Access BRC's technical library: installation procedures", "description=\"Access BRC's technical library - installation procedures")
...
video_list = """                    <ul className="space-y-3 mb-8 text-slate-300 font-light text-sm">
...
content = re.sub(r'<Link href="/contact\?subject=Video\+Training\+Access.*?</Link>', video_list, content, flags=re.DOTALL)
...
cta = """                  <div className="relative z-10 flex flex-col gap-4 w-full md:w-auto shrink-0">
...
content = re.sub(r'<div className="relative z-10 flex flex-col gap-4 w-full md:w-auto shrink-0">.*?</div>\s+</div>', cta + '\n                </div>', content, flags=re.DOTALL)


popular = """                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 md:gap-4">
                    {[
...
content = re.sub(r'<div className="grid grid-cols-1 lg:grid-cols-3 2xl:grid-cols-4 gap-3 md:gap-4">.*?</div>\s+</motion.div>', popular + '\n                </motion.div>', content, flags=re.DOTALL)

with open('src/features/technical-resources/components/TechnicalResourcesClient.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
